        ...
        const formattedPrescriptions = await Promise.all(
            allPrescriptions.flatMap(async item => {
                ...
                if (item.predate2 && item.predate2 !== 0) {
                    rows.push({ id: item._id, precount:item.precount, type: 'predate2', check: 'prei2', value: item.predate2, pname, pline, pphone, pid: item.pid, preday: item.preday , prei: item.prei2});
                }
                if (item.predate3 && item.predate3 !== 0) {
                    rows.push({ id: item._id, precount:item.precount, type: 'predate3', check: 'prei3', value: item.predate3, pname, pline, pphone, pid: item.pid, preday: item.preday , prei: item.prei3});
                }
                return rows;
            })
        );
